...
`licenses.txt` is refreshed once a day by [scraper/scrape.py](scraper/scrape.py), run from GitHub Actions ([.github/workflows/scrape.yml](.github/workflows/scrape.yml)). A commit is made only when the licence table on the Ministry's page changes, with the page's "Last updated" date as the commit subject (YYYYMMDD). See [scraper/README.md](scraper/README.md) for details and for running it from a server instead.
